...
import { fetchEvents } from "@/api/ground";
import { isoDateToYmd } from "@/lib/utils";
import { GS_EVENT_NAMES, GS_MODE_NAMES, type GsEventRow } from "@/types/ground";
...
export function EventsQueryCard() {
  ...
  const [rows, setRows] = useState<GsEventRow[]>([]);
  const [busy, setBusy] = useState(false);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setBusy(true);
    const result = await fetchEvents(serial, isoDateToYmd(start), isoDateToYmd(end));
    setBusy(false);
    if (result.status === "ok") {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Get events</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-3">
        ...
        <div className="max-h-64 overflow-y-auto">
          {rows.length === 0 ? (
            <p className="text-sm text-muted-foreground">{status ? "No events in that range." : ""}</p>
          ) : (
            <div className="flex flex-col gap-1.5">
              {rows.map((r, i) => (
                <div key={i} className="rounded-md bg-muted px-2.5 py-1.5 font-mono text-xs">
                  {r.ymd} {r.hms} {GS_EVENT_NAMES[r.eventType] ?? "?"} value={r.eventValue} mode=
                  {GS_MODE_NAMES[r.mode] ?? "?"}
                </div>
              ))}
            </div>
          )}
