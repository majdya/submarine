#ifndef GROUND_STATION_HTTP_SERVER_H
#define GROUND_STATION_HTTP_SERVER_H
...
// A minimal, dependency-free embedded HTTP server for the Ground Station's
// read-only web dashboard - a verbatim copy of central-computer's own
// http_server.h/.cpp (down to the exact wire handling), just renamed into
// this project's own namespace, since it's a fully self-contained,
// dependency-free module with nothing central-computer-specific in it.
// Deliberately hand-rolled (raw sockets, POSIX/Win32 behind one interface)
// rather than pulling in a library, since this project's build has no
// network access to fetch one. It is not a general-purpose web server:
// single request per connection (no keep-alive), GET query strings and
// POST bodies are both simple key=value pairs
// (application/x-www-form-urlencoded) so the dashboard never needs a JSON
// *parser* - only json_util.h's JSON *writer*, and even that only for a
// small error-wrapper shape, since every successful response here is
// really just the Central Computer's own JSON relayed verbatim (see
// gs_dashboard_api.h). That is enough for a local single-user dashboard
// talking to its own page's JS.
struct HttpRequest {
  std::string method;
...
#endif  // GROUND_STATION_HTTP_SERVER_H
